...
(defrecord HTTPServer [started? server-settings server store mqtt-client db database jam-manager roomdb middleware-data]
  component/Lifecycle
  (start [this]
    (if started?
      this
      (do (log/info "Starting HTTPServer")
          (let [session-backend (backends/session {:unauthorized-handler unauthorized-handler})
                ...
                server (create-server server-settings middleware-data)]
            (assoc this
                   :started? true
                   :server server
